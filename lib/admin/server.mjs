...
import { saveVkSettings } from "./connector-secret-store.mjs";
import { syncVkFeed } from "./vk-connector.mjs";
import { listAdminVkFeed, listPublicVkFeed, updateVkFeedItem } from "./vk-feed-store.mjs";
...
async function route(request, response) {
  ...
  if (request.method === "GET" && url.pathname === "/api/content/vk") {
    return send(response, 200, { ok: true, ...(await listPublicVkFeed(config)) }, { "Cache-Control": "public, max-age=60, stale-while-revalidate=300" });
  }
  ...
  if (request.method === "GET" && url.pathname === "/api/admin/integrations") {
    requireSession(request);
    return send(response, 200, { ok: true, integrations: await listIntegrationStatuses(config) });
  }
  if (request.method === "GET" && url.pathname === "/api/admin/vk/items") {
    requireSession(request);
    return send(response, 200, { ok: true, ...(await listAdminVkFeed(config)) });
  }
  if (request.method === "POST" && url.pathname === "/api/admin/vk/sync") {
    const session = requireMutationAuth(request);
    return send(response, 200, { ok: true, ...(await syncVkFeed(config, session.username)), externalRequestMade: true });
  }
  if (request.method === "PUT" && url.pathname === "/api/admin/integrations/vk/settings") {
    const session = requireMutationAuth(request);
    const body = await readJson(request);
    return send(response, 200, { ok: true, settings: await saveVkSettings(config, session.username, body) });
  }
  const vkItemMatch = url.pathname.match(/^\/api\/admin\/vk\/items\/([^/]+)$/);
  if (request.method === "PATCH" && vkItemMatch) {
    const session = requireMutationAuth(request);
    const body = await readJson(request);
    const expectedRevision = asValidation(() => validateExpectedRevision(body?.revision));
    const catalog = await listProducts(config);
    const allowedProductIds = new Set(catalog.products.filter((product) => product.active).map((product) => product.id));
    return send(response, 200, {
      ok: true,
      ...(await updateVkFeedItem(config, session.username, decodeURIComponent(vkItemMatch[1]), expectedRevision, body?.item || {}, allowedProductIds)),
    });
  }
