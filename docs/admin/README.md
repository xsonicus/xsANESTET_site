...
- значения не записываются в `catalog.json` или `admin-audit.jsonl`;
...
Сейчас endpoint проверки выполняет безопасный configuration preflight. Если переменных не хватает, он возвращает HTTP 409 и не делает исходящий запрос. Если реквизиты полные, но конкретный protocol adapter ещё не установлен, он возвращает HTTP 501 `adapter_pending` и также не делает исходящий запрос. Это намеренная граница: соединение нельзя считать успешным без официального контракта API, тестового аккаунта и проверки ответа конкретной системы.
